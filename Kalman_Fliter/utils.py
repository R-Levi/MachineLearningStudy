...
def plot_one_box(xyxy,img,color=(0,200,0),target=False):
    ...
    if target:
        color = (0,0,255)
    cv.rectangle(img,xy1,xy2,color,1,lineType=cv.LINE_AA)

def draw_trace(img,trace_list):
    for i,item in enumerate(trace_list):
        if i==0:
            continue
        cv.line(img,(trace_list[i][0], trace_list[i][1]), (trace_list[i - 1][0], trace_list[i - 1][1]),color=(255,255,0),thickness=3)
